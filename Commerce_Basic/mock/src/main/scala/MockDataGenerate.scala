...
import com.huanfion.commons.model.{ProductInfo, UserInfo, UserVisitAction}
import com.huanfion.commons.utils.{DateUtils, StringUtils}
...
object MockDataGenerate {
    ...
    for (i <- 0 to 100) {
      ...
      for (j <- 0 to 10) {
        ...
        for (k <- 0 to random.nextInt(100)) {
          ...
          rows += UserVisitAction(date, userid, sessionid, pageid, actiontime, searchKeyword,
  ...
  /**
    ...
    * @param df
    */
  def insertHive(spark: SparkSession, tablename: String, df: DataFrame) = {
  ...
  def main(args: Array[String]): Unit = {
    ...
    //创建sparksession实例
    val spark = SparkSession.builder().config(sparkConf).config("hive.metastore.uris", "thrift://master:9083").enableHiveSupport().getOrCreate()
    //模拟数据
    ...
    //将数据转换成df保存到hive
    insertHive(spark, USER_VISIT_ACTION_TABLE, userVisitActionRdd.toDF())
    insertHive(spark, USER_INFO_TABLE, userInfoRdd.toDF())
    insertHive(spark, PRODUCT_INFO_TABLE, productInfoRdd.toDF())
    spark.stop()
    spark.close()
  }
}
