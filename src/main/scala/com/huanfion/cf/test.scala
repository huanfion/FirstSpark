...
object UserBase {
  def main(args: Array[String]): Unit = {
    ...
    //    udata.filter("user_id='196'").show(5)
    import spark.implicits._
    //    |a|= sqrt(a1^2+a2^2+...+an^2)
    val userScoreSum = udata.rdd.map(x=>(x(0).toString,x(2).toString)).groupByKey().mapValues(x=> sqrt(x.toArray.map(rating=>pow(rating.toDouble,2)).sum)).toDF("user_id","rating_sqrt_sum")

    //    1.1 item->user倒排表
    val df = udata.selectExpr("user_id as user_v","item_id as item_id","rating as rating_v")
    val df_decare = udata.join(df,"item_id").filter("cast(user_id as long) <> cast(user_v as long)")
    //    df.selectExpr("user_id").distinct().filter("cast(user as long)<=5")
    //    计算两个用户在一个item下的评分的乘积，consine举例的分子的一部分
    //    dot
    import org.apache.spark.sql.functions._
    val product_udf = udf((s1:Int,s2:Int)=>s1.toDouble*s2.toDouble)
    val df_product = df_decare.withColumn("rating_product",product_udf(col("rating"),col("rating_v"))).select("user_id","user_v","rating_product")

    //    求和，计算完整的分子部分
    val df_sim_group = df_product.groupBy("user_id","user_v").agg("rating_product"->"sum").withColumnRenamed("sum(rating_product)","rating_dot")

    val userScoreSum_v = userScoreSum.selectExpr("user_id as user_v", "rating_sqrt_sum as rating_sqrt_sum_v")
    val df_sim = df_sim_group.join(userScoreSum,"user_id").join(userScoreSum_v,"user_v").selectExpr("user_id","user_v", "rating_dot/(rating_sqrt_sum*rating_sqrt_sum_v) as cosine_sim")

    //  2.  获取相似用户的物品集合
    //    2.1取得前n个相似用户
    val df_nsim = df_sim.rdd.map(x=> (x(0).toString,(x(1).toString,x(2).toString))).groupByKey().mapValues{x=> x.toArray.sortWith((x,y)=>x._2>y._2).slice(0,10) }.flatMapValues(x=>x).toDF("user_id","user_v_sim").selectExpr("user_id","user_v_sim._1 as user_v","user_v_sim._2 as sim")

    //    2.2获取用户的物品集合进行过滤     273|[328_3, 345_3, 31...|
    val df_user_item = udata.rdd.map(x=>(x(0).toString,x(1).toString+"_"+x(2).toString)).groupByKey().mapValues(x=>x.toArray).toDF("user_id","item_rating_arr")
    //    过滤
    ...
    val df_filter_item = df_gen_item.withColumn("filtered_item", filter_udf(col("item_rating_arr"),col("item_rating_arr_v"))).select("user_id","sim","filtered_item")
